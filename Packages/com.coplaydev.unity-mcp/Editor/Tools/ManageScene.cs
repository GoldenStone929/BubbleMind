...
namespace MCPForUnity.Editor.Tools
{
    ...
    public static class ManageScene
    {
        ...
        private static bool TryNormalizeProjectScenePath(
            string input,
            ...
            out string error)
        {
            ...
            string normalized = AssetPathUtility.NormalizeSeparators(input.Trim());
            if (Path.IsPathRooted(normalized) || normalized.Contains(':'))
            {
                error = "Scene paths must be project-relative paths under Assets/.";
                return false;
            }
            ...
            if (!candidate.Equals(assetsRoot, StringComparison.OrdinalIgnoreCase) &&
                !candidate.StartsWith(assetsPrefix, StringComparison.OrdinalIgnoreCase))
            {
                error = "Scene paths may not escape the project's Assets directory.";
                return false;
            }

            string relative = candidate.Length == assetsRoot.Length
                ? string.Empty
                : candidate.Substring(assetsPrefix.Length);
            assetPath = string.IsNullOrEmpty(relative)
                ? "Assets"
                : "Assets/" + AssetPathUtility.NormalizeSeparators(relative);
            return true;
        }

        private static bool IsSafeSceneName(string name)
        {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            {
                return new ErrorResponse("Scene 'name' must be a file name without path separators.");
            }

            if (!TryNormalizeProjectScenePath(path, out string normalizedInputPath, out string pathError))
            {
                return new ErrorResponse(pathError);
            }

            // Ensure path is relative to Assets/, removing the validated leading "Assets/".
            string relativeDir = normalizedInputPath ?? string.Empty;
            ...
            string fullPathDir = Path.GetFullPath(Path.Combine(Application.dataPath, relativeDir));
            ...
            switch (action)
            {
                ...
                case "load":
                    ...
                    if (string.IsNullOrEmpty(loadPath) && !string.IsNullOrEmpty(normalizedInputPath))
                        loadPath = normalizedInputPath;
