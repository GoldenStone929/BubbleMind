...
namespace MCPForUnity.Editor.Windows
{
    public class MCPForUnityEditorWindow : EditorWindow
    {
        ...
        public void CreateGUI()
        {
            ...
            // This embedded package is operated through the repository-scoped Codex
            // configuration. Do not restore the upstream settings UI because creating it
            // probes user-level client configs, global EditorPrefs, and update endpoints.
            if (!ProjectIsolation.AllowGlobalEditorPrefs)
            {
                rootVisualElement.Clear();
                rootVisualElement.Add(new Label(
                    "MCP for Unity is running in project-isolated mode. " +
                    "Use Tools > Generic Gacha RPG > Ensure Unity MCP Bridge."));
                guiCreated = true;
                return;
            }
        ...
        {
            if (!string.Equals(
                    Environment.GetEnvironmentVariable("UNITY_MCP_ALLOW_UPDATE_CHECKS"),
                    "1",
