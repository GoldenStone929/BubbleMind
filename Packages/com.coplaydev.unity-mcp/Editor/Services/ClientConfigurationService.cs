...
namespace MCPForUnity.Editor.Services
{
    ...
    public class ClientConfigurationService : IClientConfigurationService
    {
        ...
        {
            EnsureClientConfigAccessAllowed();
        ...
        {
            if (!IsClientConfigAccessAllowed()) return false;
        ...
        private static bool IsClientConfigAccessAllowed() => string.Equals(
            Environment.GetEnvironmentVariable("UNITY_MCP_ALLOW_CLIENT_CONFIG_WRITES"),
            "1",
            StringComparison.Ordinal);

        private static void EnsureClientConfigAccessAllowed()
        {
            if (!IsClientConfigAccessAllowed())
            {
                throw new InvalidOperationException(
                    "User-level MCP client configuration is disabled in project-isolated mode.");
            }
        }
