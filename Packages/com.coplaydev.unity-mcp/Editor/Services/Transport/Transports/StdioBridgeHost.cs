...
namespace MCPForUnity.Editor.Services.Transport.Transports
{
    ...
    public static class StdioBridgeHost
    {
        ...
        {
            if (!ProjectIsolation.AllowGlobalEditorPrefs) return false;
        ...
        {
            if (!string.Equals(
                    Environment.GetEnvironmentVariable("UNITY_MCP_ALLOW_STDIO_AUTOSTART"),
                    "1",
            ...
            lock (startStopLock)
            {
                ...
                try
                {
                    ...
                    catch (SocketException se) when (se.SocketErrorCode == SocketError.AddressAlreadyInUse)
                    {
                        ...
                        try
                        {
                            if (ProjectIsolation.AllowGlobalEditorPrefs)
                            {
        ...
        public static void Stop()
        {
            ...
            try
            {
                string dir = GetRequiredProjectStatusDirectory();
        ...
        public static void WriteHeartbeat(bool reloading, string reason = null)
        {
            try
            {
                ...
                bool projectScopedTools = string.Equals(
                    Environment.GetEnvironmentVariable("UNITY_MCP_PROJECT_SCOPED_TOOLS"),
                    "1",
                    StringComparison.Ordinal) || EditorPrefs.GetBool(
        ...
        private static string GetRequiredProjectStatusDirectory()
        {
            string statusDirectory = Environment.GetEnvironmentVariable("UNITY_MCP_STATUS_DIR");
            if (string.IsNullOrWhiteSpace(statusDirectory))
            {
                throw new InvalidOperationException(
                    "UNITY_MCP_STATUS_DIR is required by the project-isolated MCP package.");
            }

            string projectRoot = Path.GetFullPath(Path.Combine(Application.dataPath, ".."))
                .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
            string fullStatusDirectory = Path.GetFullPath(statusDirectory)
                .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
            string projectPrefix = projectRoot + Path.DirectorySeparatorChar;
            if (!fullStatusDirectory.StartsWith(projectPrefix, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException(
                    $"UNITY_MCP_STATUS_DIR must remain inside the Unity project: {fullStatusDirectory}");
            }

            return fullStatusDirectory;
        }
    }
}
