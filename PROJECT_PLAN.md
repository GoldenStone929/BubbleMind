...
> 计划版本：v2.9-home-shell-reference-session-02<br>
> 当前状态：阶段 6.11 已完成；完整离线系统壳保持可玩，参考游戏第二轮只读审计已转化为中性知识库，同时共享视觉令牌、全幅主页与明亮 App Shell 已完成首轮表现升级<br>
> 最近更新：2026-07-16
...
8. 以职业规则决定攻击距离：Tank / Assassin 为 2 格，其余职业为 10 格；单位只接近到自身最大攻击距离并在那里停位；
9. 受击时有视觉反馈；
10. HP 为零时死亡并停止行动；
11. 死亡单位不可再被选为目标；当前目标死亡后按当前位置重选最近敌人；
12. 受击、普攻和技能表现不得把角色世界位置重置到出生槽；
13. Ember Striker 的技能槽 2 在第 5 秒选择敌方存活后排，瞬移到 2 格射程内并持续锁定攻击；
14. Catherine 的 `Wind Wheel: Break` 击退距离为 5 格；
15. 全灭或超时时生成稳定结果。
...
- 战场主轴长度为 20 格；攻击距离由职业统一约束：Tank / Assassin 为 2，Support / Ranged / Mage 为 10；移动只关闭到攻击距离边界，不额外贴近目标，所有位移都限制在战场边界内。
- 目标选择、伤害、治疗和技能效果优先由通用规则/数据驱动；确有独特机制时使用独立 BattleKit 常量与通用事件表达，避免把表现逻辑写进模拟层。
...
- Ember Striker 在第 5 秒以技能槽 2 瞬移到敌方后排，并持续攻击该目标直到其死亡。
...
| 2026-07-16 | 参考审计 Session 02 只沉淀中性系统关系 | 用户要求继续查看已登录参考游戏并完整记录页面连接，但 BubbleMind 必须保持 clean-room 原创 | 本机 Artifacts 保留 67 张连续证据；版本库只记录模块族、状态模式、实体关系和 BubbleMind 原创映射，专有截图、长文案、角色数值与布局不进入需求或运行时 |
| 2026-07-16 | 路线 A 的共享 UI 与主页阶段先于正式模型继续推进 | 用户决定模型最后处理，并要求先完成完整主页面、角色页入口、抽卡卡面和顺滑 UI | 以现有原创星渊观测台作为全幅 Hub；新增共享视觉令牌、语义热点、动态收藏/编队状态和明亮 App Shell，保留全部路由、对象名、服务、存档与确定性战斗契约 |
