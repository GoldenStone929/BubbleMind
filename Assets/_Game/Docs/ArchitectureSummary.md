...
- Tank / Assassin 的攻击距离固定为 1，Support / Ranged / Mage 固定为 5；单位每 Tick 按移动速度接近锁定目标，在自身最大射程边界停止，目标存活期间不换锁，死亡后才按当前位置选择最近敌人。
...
- `DemoPlayModeSmokeOrchestrator`：运行完整页面流程，并检查 10 个世界怒气条和 5v5 战斗事件契约。
